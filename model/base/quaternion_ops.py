...
def quaternion_conv(input, r_weight, i_weight, j_weight, k_weight, bias, stride,
    ...
    """
    # bsz, ch, ha, wa, hb, wb = input.size()
    # input = input.permute(0, 4, 5, 1, 2, 3).contiguous().view(bsz, -1, ha, wa)
    ...
    output = convfunc(input, cat_kernels_4_quaternion, bias, stride, padding, dilatation, groups)
    # output = output.view(bsz, ch, hb, wb, ha, wa).permute(0, 1, 4, 5, 2, 3).contiguous()
    # bsz, ch, _, _ = output.size()
    # output = output.view(bsz, hb, wb, int(ch/4), ha, wa).permute(0, 3, 4, 5, 1, 2).contiguous()
    return output
...
def quaternion_transpose_conv(input, r_weight, i_weight, j_weight, k_weight, bias, stride,
    ...
    output = convfunc(input, cat_kernels_4_quaternion, bias, stride, padding, output_padding, groups, dilatation)
    # output = convfunc(input, cat_kernels_4_quaternion, bias, stride, padding, dilatation, groups)
    # output = output.view(bsz, ch, hb, wb, ha, wa).permute(0, 1, 4, 5, 2, 3).contiguous()
    # bsz, ch, ha_n, wa_n = output.size()
    # output = output.view(bsz, hb, wb, int(ch / 4), ha_n, wa_n).permute(0, 3, 4, 5, 1, 2).contiguous()
    return output
...
def quaternion_conv_rotation(input, zero_kernel, r_weight, i_weight, j_weight, k_weight, bias, stride,
    ...
    output = convfunc(input, global_rot_kernel, bias, stride, padding, dilatation, groups)
    # output = convfunc(input, cat_kernels_4_quaternion, bias, stride, padding, dilatation, groups)
    # output = output.view(bsz, ch, hb, wb, ha, wa).permute(0, 1, 4, 5, 2, 3).contiguous()
    bsz, ch, ha_n, wa_n = output.size()
    output = output.view(bsz, hb, wb, int(ch / 4), ha_n, wa_n).permute(0, 3, 4, 5, 1, 2).contiguous()
